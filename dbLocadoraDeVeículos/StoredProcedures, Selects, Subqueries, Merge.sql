use dbLocadoraVeiculo
	...
	begin
		...
		else if not exists(select idVeiculo from tbVeiculo where placaVeiculo = @placaVeiculo) begin print('Veículo não encontrado!') end
		else
			begin
	...
	-- E --
		...
		select  origemVeiculo as 'Procedência', count(idVeiculo) as 'Qtnd. de veículos locados' from tbVeiculo
			group by origemVeiculo
