use bdEscola
	...
	-- 1 --
	create procedure spBuscaAluno @cpfAluno varchar(15)
	as
	begin
		...
		else begin print('Aluno não encontrado!') end
	end
	exec spBuscaAluno "535.092.285-07"
	...
	-- 5 --
	create procedure spMatriculaAluno @cpfAluno varchar(15), @nomeCurso varchar(20)
	as
	begin
		if exists(select cpfAluno from tbAluno where cpfAluno like @cpfAluno) and exists(select nomeCurso from tbCurso where nomeCurso like @nomeCurso)
			begin
				insert into tbMatricula(codAluno, codTurma, dataMatricula)
				values((select codAluno from tbAluno where cpfAluno like @cpfAluno)
	...
	exec spMatriculaAluno '535.092.285-07', 'Alemão'
